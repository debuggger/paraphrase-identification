...
class TextCNNTest(object):
    ...
    def __init__(
      ...
      embedding_size, filter_sizes, num_filters, checkpoint_file, session, l2_reg_lambda=0.0):
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.name_scope("conv-maxpool-1-%s" % filter_size):
                # Convolution Layer
                print '-->',filter_size, num_filters, embedding_size
                filter_shape = [filter_size, embedding_size, 1, num_filters]
                print filter_shape
                W = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.1), name="W")
                b = tf.Variable(tf.constant(0.1, shape=[num_filters]), name="b")
                saver = tf.train.Saver({"conv-maxpool-1-%s/W" % filter_size: W, "conv-maxpool-1-%s/b" % filter_size: b})
            ...
            with tf.name_scope("conv-maxpool-2-%s" % filter_size):
                    ...
                    W = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.1), name="W")
                    b = tf.Variable(tf.constant(0.1, shape=[num_filters]), name="b")
                    saver = tf.train.Saver({"conv-maxpool-2-%s/W" % filter_size: W, "conv-maxpool-2-%s/b" % filter_size: b})
                    saver.restore(session, checkpoint_file)
                    conv = tf.nn.conv2d(
                        self.x2,
                        W,
