        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.name_scope("conv-maxpool-1a-%s" % filter_size):
                ...
                pooled = tf.nn.avg_pool(
                    h,
                    ksize=[1, sequence_length - filter_size + 1, 2, 1],
                    strides=[1, sequence_length - filter_size + 1, 2, 1],
                    padding='VALID',
                    name="pool")
                topk_pool = tf.nn.top_k(tf.transpose(pooled, [0, 3, 2, 1]), sequence_length/2, False)
                pooled_outputs_1a.append(tf.transpose(topk_pool, [0, 3, 2, 1]))
            ...
            with tf.name_scope("conv-maxpool-2a-%s" % filter_size):
                ...
                pooled = tf.nn.avg_pool(
                    ...
                    name="pool")
                topk_pool = tf.nn.top_k(tf.transpose(pooled, [0, 3, 2, 1]), sequence_length/2, False)
                pooled_outputs_2a.append(tf.transpose(topk_pool, [0, 3, 2, 1]))
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.name_scope("conv-maxpool-1b-%s" % filter_size):
                ...
                pooled = tf.nn.avg_pool(
                    ...
                    name="pool")
                topk_pool = tf.nn.top_k(tf.transpose(pooled, [0, 3, 2, 1]), sequence_length/2, False)
                pooled_outputs_1b.append(tf.transpose(topk_pool, [0, 3, 2, 1]))
            ...
            with tf.name_scope("conv-maxpool-2b-%s" % filter_size):
                ...
                pooled = tf.nn.avg_pool(
                    ...
                    name="pool")
                topk_pool = tf.nn.top_k(tf.transpose(pooled, [0, 3, 2, 1]), sequence_length/2, False)
                pooled_outputs_2b.append(tf.transpose(topk_pool, [0, 3, 2, 1]))
