...
class TextCNNDeep(object):
    ...
    def __init__(
      ...
      embedding_size, filter_sizes, num_filters_1, num_filters_2, checkpoint_file, session, l2_reg_lambda=0.0):
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.name_scope("conv-maxpool-1a-%s" % filter_size):
                ...
                saver = tf.train.Saver({"conv-maxpool-1a-%s/W" % filter_size: W, "conv-maxpool-1a-%s/b" % filter_size: b})
            ...
            with tf.name_scope("conv-maxpool-2a-%s" % filter_size):
                    ...
                    W = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.1), name="W")
                    b = tf.Variable(tf.constant(0.1, shape=[num_filters_1]), name="b")
                    saver = tf.train.Saver({"conv-maxpool-2a-%s/W" % filter_size: W, "conv-maxpool-2a-%s/b" % filter_size: b})
                    saver.restore(session, checkpoint_file)
                    conv = tf.nn.conv2d(
                        self.x2,
                        W,
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.name_scope("conv-maxpool-1b-%s" % filter_size):
                ...
                saver = tf.train.Saver({"conv-maxpool-1b-%s/W" % filter_size: W, "conv-maxpool-1b-%s/b" % filter_size: b})
                saver.restore(session, checkpoint_file)
                conv = tf.nn.conv2d(
            ...
            with tf.name_scope("conv-maxpool-2b-%s" % filter_size):
                    ...
                    W = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.1), name="W")
                    b = tf.Variable(tf.constant(0.1, shape=[num_filters_2]), name="b")
                    saver = tf.train.Saver({"conv-maxpool-2b-%s/W" % filter_size: W, "conv-maxpool-2b-%s/b" % filter_size: b})
                    saver.restore(session, checkpoint_file)
                    conv = tf.nn.conv2d(
        ...
        with tf.name_scope("dropout"):
            self.h_drop = tf.nn.dropout(self.h_pool_flat, 1.0)
        ...
        with tf.name_scope("output"):
            ...
            saver = tf.train.Saver({"W": W, "output/b": b})
            saver.restore(session, checkpoint_file)
